...
class Broker {
	 ...
	 * This registers a callback on a channel with optional *options*. <br/>eg:<br/>
	 ...
	 ```javascript
	 broker.on('gateway:processCC', {force:true, priority:1}, mycallback);
	 // OR:
	 broker.on('gateway:processCC', {force:true, priority:10}, mysubid);
	 ```
...
a	 * @param {Object} [options] - An object with options that get passed back to the callback. Some parameters are used 
	 ...
	 * This emits an event on a channel id with optional data. Subscriptions are called in order of channel id exactness 
	 ...
	 ```
	 {data:*, channelId:String, subscription:Object}
	 ```
	 Includes aliases: *emit*, *publish*, and *trigger*. 
	 *
	 * @param {string} channelId - The channel id which will be triggered (```a:b:c```).
		...
		let nextSubscriber = function* () {
			...
			for(let chanId of list) {
				...
				for(let sub of channels[chanId]) {
					...
					prom = Promise.resolve(sub.callback.call(context, event));
